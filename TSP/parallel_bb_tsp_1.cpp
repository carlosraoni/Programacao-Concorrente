...
vector< Tour > initialTasks; // Tarefas iniciais
vector< vector<Tour> > tasksPerWorker; // Cada worker terá sua fila de tarefas (tour's parciais a serem expandidos)
...
void createInitialTasks(){
	...
	for(int i=0; i<NWORKERS; i++){
		tasksPerWorker.push_back(vector<Tour>());
	}

	int totalTasks = initialTasks.size();
	cout << "Total Initial Tasks = " << totalTasks << endl;
	int currWorker = 0;
	for(int i=0; i<totalTasks; i++){
		tasksPerWorker[currWorker].push_back(initialTasks[i]);
		currWorker = (currWorker + 1) % NWORKERS;
	}
	for(int i=0; i<NWORKERS; i++){
		cout << "Task for worker " << i << " = " << tasksPerWorker[i].size() << endl;
	}
}
...
void * worker(void * arg){
	...
	vector< pair<double, Tour> > candidates;
	vector< Tour > tasks = tasksPerWorker[threadId];
	int nTasks = tasks.size();
    for(int i=0; i<nTasks; i++){
    	double lb = calculateLowerBound(tasks[i]);
    	if(lb < getUpperBound()){
    		candidates.push_back(pair<double, Tour>(lb, tasks[i]));
    	}
    }
